...
// there is no JS unit-test harness in this codebase today (no jsdom, no node-based test runner);
// adding one is on the 2.0 roadmap (`docs/plan.md`). The C++ frame parser at
// src/core/ImprovFrame.h + test/test_improv_frame.cpp is the equivalent test
// for the only piece of this work that runs on the device side.
...
// Each release's `assets[]` contains both manifest-<board>.json and
// firmware-<board>-v<ver>(.bin|-bootloader.bin|-partition-table.bin|-ota-data.bin).
// The picker needs: per-board → {manifestUrl, binaryUrl}. Manifest URL drives
// ESP Web Tools (web installer); binary URL drives /api/firmware/url (device OTA).
function parseBoardsFromAssets(assets, tag) {
    if (!assets) return [];
    ...
    for (const a of assets) {
        ...
        if (m) {
            const board = m[1];
            const entry = boards.get(board) || { board, manifestUrl: null, binaryUrl: null };
            entry.manifestUrl = a.browser_download_url;
            boards.set(board, entry);
        }
    }
    for (const a of assets) {
    ...
    // Only return boards that have BOTH a manifest and a binary — partial uploads
    // (mid-release-publish race) shouldn't appear in the dropdown.
    return Array.from(boards.values()).filter(b => b.manifestUrl && b.binaryUrl);
}
...
// Bespoke rule for projectMM's board keys: strip the `-eth*` suffix from both
// sides; equal identities are mutually OTA-compatible. So `esp32`, `esp32-eth`,
// and `esp32-eth-wifi` can all flash each other (same physical ESP32 silicon;
// the variant decides which radios are compiled in). `esp32s3-n16r8` is only
// compatible with itself (different chip family AND different partition table).
// Web installer passes ownBoardKey=null → all candidates compatible.
function isCompatible(ownBoardKey, candidateBoardKey) {
    if (!ownBoardKey) return true;
    const strip = b => b.replace(/-eth.*$/, "");
    return strip(ownBoardKey) === strip(candidateBoardKey);
}
...
function render(state) {
    ...
    // production release. The compatibility filter at the board step handles
    ...
    function refreshBoardDropdown() {
        ...
        if (!r) {
            boardEl.innerHTML = `<option value="">—</option>`;
            installBtn.disabled = true;
            return;
        }
        const compatible = (r.boards || []).filter(b => isCompatible(state.ownBoardKey, b.board));
        if (compatible.length === 0) {
            // Distinguish two no-match reasons: device reports its board but
            // this release has nothing matching it, vs device's board is
            // "unknown" (build didn't propagate MM_BOARD_NAME) and the
            // compatibility filter rejects everything. The latter is a build
            // bug — surface it so a developer can spot it.
            const reason = state.ownBoardKey === "unknown"
                ? "device board is 'unknown' — rebuild firmware with MM_BOARD_NAME set"
                : "no compatible boards in this release";
            boardEl.innerHTML = `<option value="">— ${reason} —</option>`;
            installBtn.disabled = true;
            return;
        }
    ...
    installBtn.addEventListener("click", async () => {
        ...
        try {
            await state.onInstall(state.board, entry.manifestUrl, entry.binaryUrl);
...
export const releasePicker = {
     ...
     * @param {string|null} opts.ownBoardKey - device's MM_BOARD_NAME for OTA
     *   compatibility filtering, or null for the web installer (no filter).
     * @param {(board: string, manifestUrl: string, binaryUrl: string) => Promise<void>} opts.onInstall
     *   - Called when the user clicks Install. The picker doesn't decide how
     *     to install — the caller decides.
     */
    async init({ container, ownBoardKey, onInstall }) {
        const state = makeState();
        state.container = container;
        state.ownBoardKey = ownBoardKey || null;
        ...
        // Normalise: enrich each release with its parsed boards list.
        state.releases = data.map(r => ({
            ...
            html_url: r.html_url,
            boards: parseBoardsFromAssets(r.assets, r.tag_name),
        }))
        // Drop releases with zero usable boards (no firmware-* / manifest-* assets).
