...
**ESP32 — flash from your browser.** Open <https://ewowi.github.io/projectMM/install/> in Chrome or Edge, plug the device in over USB, pick your board, click Install. After flashing, WiFi builds bring up Improv: the install page shows a "Configure Wi-Fi" dialog, you enter SSID + password, the device joins your network. If Improv doesn't appear (or you skipped it), the device boots a SoftAP named `MM-XXXX` (4 hex digits from the chip MAC) at `4.3.2.1` as a fallback. Ethernet builds get a DHCP address as soon as you plug the cable in. Reach the device via its mDNS hostname `http://<devicename>.local` (defaults to `MM-XXXX.local`; rename in the device UI to change it) or the DHCP IP from your router. Four board variants ship: `esp32` (WiFi only), `esp32-eth` (Ethernet only, Olimex pin map), `esp32-eth-wifi` (both), `esp32s3-n16r8` (S3 DevKitC-1 with the N16R8 module).
