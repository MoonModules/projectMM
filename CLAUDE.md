...
The `history/` folder is the distilled experience of years of building LED/light systems, from WLED, WLED-MM, StarLight, MoonLight, through projectMM. It contains proven patterns, memory tricks, control mechanisms, and hard-won lessons. We **study** these projects under the *Industry standards, our own code* principle in [§ Principles](#principles): read their approaches to sharpen our thinking, write everything fresh against our own architecture, never copy. Per-project credits live in the `history/` digests and the per-module "Prior art" sections.
