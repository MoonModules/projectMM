...
TEST_CASE("ControlType::List type identity + non-persistable + read-only") {
    CHECK(std::strcmp(mm::controlTypeName(mm::ControlType::List), "list") == 0);
    // Non-persistable: discovery output, rebuilt each scan — never saved/loaded,
    // which is also why the flat JSON parser never has to read a List back.
    CHECK_FALSE(mm::isPersistable(mm::ControlType::List));
    // A write to a List is a no-op that reports ReadOnly (v1 lists aren't edited).
    StubDevices src;
    mm::ControlList controls;
    controls.addList("devices", src);
    auto r = mm::applyControlValue(controls[0], "{\"devices\":\"x\"}", "devices",
                                   mm::ApplyPolicy::Clamp);
    CHECK(r == mm::ApplyResult::ReadOnly);
}
