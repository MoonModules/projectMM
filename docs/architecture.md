...
> **⏳ Temporary (remove at the board→deviceModel rename step):** the code still names the deviceModel concept `board` — the `board` control, `boards.json`, `setBoard`, the `SET_BOARD` RPC. The prose above already uses the destination name `deviceModel`; this note bridges the gap until the identifiers are renamed to match, at which point this paragraph is deleted.
