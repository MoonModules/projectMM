...
- **MCU → firmware.** The chip (classic / S3 / P4). Fixes silicon-wired facts: the RMII Ethernet pin map, native-radio presence, PSRAM — already the compile-time `platform::ethPins` / `hasI2sMic` / `hasWifi` constants in `platform_config.h`. The firmware variant *is* the MCU choice.
