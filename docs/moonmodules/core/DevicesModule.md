...
- `progress` — a progress bar of the sweep position (host 1..254). Always present (the WebSocket state push patches control *values* but not *structure*, so a hide-while-idle flag would not update live); at rest its value is 0 (empty bar).
