...
| `firmware` | read-only string | Build-time firmware variant key from `src/core/build_info.h` (`MM_FIRMWARE_NAME`): `esp32`, `esp32-eth`, `esp32-16mb`, `esp32s3-n16r8`, … for the shipped firmware variants (the full list is the `FIRMWARES` dict in `build_esp32.py`); `desktop-macos-arm64` / `desktop-windows-x64` for packaged desktop binaries; `desktop-dev` for unpackaged local desktop builds. Devices flashed before the variant collapse may still report the old `esp32-eth-wifi` key, which OTA-maps to today's `esp32`. Identifies which release asset matches the device — the same key appears in the firmware filenames published by `release.yml`. The compiled binary; the physical hardware it runs on is SystemModule's `deviceModel` control. `install-picker.js`'s `isCompatible()` reads this string. |
| `firmwarePartition` | progress (used/total) | Running app image size / total firmware (app) partition size — how full the partition is. Named to avoid colliding with the `firmware` string control (a shared `firmware` name once broke `controls.find(c => c.name === "firmware")` callers). |
