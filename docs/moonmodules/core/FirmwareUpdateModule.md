...
|---|---|---|
| `version` | read-only string | Pure semver from library.json (`MM_VERSION`) — e.g. `2.0.0` on a stable release, or a prerelease like `2.1.0-dev` on a moving/dev build. A `-dev` (or other prerelease) suffix marks a build that is ahead of the last stable release but not itself a release (semver.org §9); a clean `X.Y.Z` is a stable release. The release channel is derivable from the version (prerelease suffix → not stable), so it is not mixed into this string — the version stays a clean, machine-comparable semver, which the UI's "update available" check compares against the newest GitHub release. |
