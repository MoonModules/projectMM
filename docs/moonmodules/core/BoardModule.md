...
The Improv RPC pushes **only** the board name (`Board.board`) — it does not consult the boards.json `controls.*` payload. The full catalog fan-out lives in the HTTP fallback path described below. For the single-field case today the two transports are equivalent; when boards.json gains per-board fields (Ethernet pin maps and similar) the RPC will continue to set just the name, and the catalog fan-out will set the rest — kicked off either by the user clicking Inject in *Your devices*, or by MoonDeck on its next discover cycle (MoonDeck holds the same catalog locally for its dropdown today; extending it to fan out the full `controls.*` after a board change is a tiny extension and tracked in [docs/plan.md](../../plan.md)).
...
`controls` is the device-injectable payload — a two-level map `{ "<ModuleName>": { "<controlName>": <value>, ... }, ... }`. The shape mirrors the `/api/control` POST body the device already accepts: each leaf becomes one `{module, control, value}` write. Today only `Board.board` is populated; future per-board state (Ethernet RMII clock GPIO, MDIO/MDC pins, default module-config overrides) lands here as additional leaves and the Inject fan-out picks them up without any device-side or installer-side code change. Catalog fields (`name`, `firmwares`, `default_firmware`) stay top-level so they're never mistaken for control writes.
...
A board name that doesn't appear in the catalog renders in MoonDeck's picker as `<name> (unknown)`, and the web installer's Inject button skips the fan-out (logged in the device UI's console).
