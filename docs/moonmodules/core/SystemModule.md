...
- `board` (read-only) — build-time board key from `src/core/build_info.h` (`MM_BOARD_NAME`): `esp32`, `esp32-eth`, `esp32-eth-wifi`, `esp32s3-n16r8` for the shipped firmware variants; `desktop-macos-arm64` / `desktop-windows-x64` for packaged desktop binaries; `desktop-dev` for unpackaged local desktop builds. Read by the future OTA path to pick a matching release asset.
