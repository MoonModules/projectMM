...
- The `firmware` control is always present and non-empty (either a real firmware key from build_info.h or the fallback "unknown"). Moved here from SystemModule — the firmware card now owns firmware identity (version/build/firmware) + the partition usage.
