...
## Device discovery: mDNS (the standard) over a UDP beacon (a WLED artifact); commands split three ways

The DevicesModule refactor started reaching for a custom UDP presence beacon (WLED-compatible) and arrived, by reasoning through what foreign systems actually do, at a cleaner endpoint — **mDNS for discovery, a plugin seam for foreign ecosystems, commands split by need**. The lessons:

- **mDNS is the discovery standard; a bespoke UDP beacon is a step backward.** Every system worth discovering — WLED, ESPHome, Tasmota, Hue, Home Assistant — already *announces over mDNS-SD*. A custom UDP presence beacon is a second mechanism only we (and WLED) speak. The "devices announce themselves" goal is *exactly* mDNS. *Common patterns first* applied to a network protocol: reach for the standard the whole ecosystem runs.
- **The mDNS QUERY RATE is the thing to control, not blocking-vs-async.** Bench testing the refactor: firing an mDNS query every render-loop tick exhausted the IDF mDNS task's internal pool — it logged `mdns_querier: Cannot allocate memory` *with 33 MB heap free* (its own fixed buffers, not real OOM) and **destabilised the device's networking**, so peers appeared to drop off the LAN. The fix is throttling to one query every few seconds, round-robined across the service types (the same cadence the old code used). A blocking ~120 ms query off the render hot path is fine; querying *too often* is not. An earlier "non-blocking async listener" detour (holding the IDF search handle across ticks) both raced a crash AND settled on the fastest responder, missing slower peers — the blocking `mdns_query_ptr` on a slow cadence is the proven path.
- **Misattributing a self-inflicted bug to "the network" or "the chip" wastes hours — read the device's own serial first.** Chasing this, I twice concluded wrongly: "the test network filters multicast" and "ESP32↔ESP32 mDNS is an IDF limitation." Both were my rate-limit bug destabilising the boards. The product owner's logic cut through it: *if the WLED app discovers WLED devices (which run on classic ESP32), then classic-ESP32 mDNS demonstrably propagates — so an invisible projectMM device is OUR bug, not the chip's.* When hardware behaves oddly, the device's serial log (which showed the mDNS alloc errors) is the first evidence, not a network theory.
- **WLED's UDP-for-discovery is a historical artifact, and a cautionary tale about transport choice.** WLED's port 21324 bundles three jobs: discovery (the artifact — WLED *also* has `_wled._tcp` mDNS, so its UDP discovery is redundant), state-sync, and live pixels. Its "messages sometimes didn't arrive" pain came from treating UDP *state-sync* as reliable. Don't copy a system's transport choices wholesale; ask what each message *needs*.
- **Commands split three ways by two questions — must it arrive? is it latency-critical? — not "all REST".** Discovery → **mDNS**. Must-arrive + latency-tolerant config (brightness, presets, OTA) → **REST/TCP** (~10–50 ms, delivery guaranteed). Latency-critical + lossy-OK (time sync for synchronized effects, live pixels) → **UDP** (~0.5–1 ms, broadcast to N at once; REST would be 10–100× too slow and would visibly de-sync). The trap is routing a sync pulse over REST because "it's a command".
- **A plugin seam makes "interop" our own industry-standard hook-in point.** Foreign systems hook in as `DevicePlugin`s (the adapter pattern), not hardcoded branches — one file per ecosystem, no core edit. The discovery half ships now (projectMM + WLED); the command half is a reserved extension (*concrete first, abstract later*). The seam deliberately allows **hub-shaped** plugins (Hue: a bridge → child resources + auth), not just flat devices — designing that room in now (without building it) is what lets the medium/hard tiers land additively. This is a product differentiator: one UI across WLED + ESPHome + Hue, incrementally.
