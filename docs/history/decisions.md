...
**Designed fresh from the datasheet + textbook DSP, not from a prior project.** Per the product owner: don't trace WLED-MM (or any existing controller) for naming, structure, or functionality — build something independent. So the choices are the vanilla, industry-recognised defaults: a **Hann window** (the standard general-purpose FFT window), RMS for level, a plain geometric (equal-ratio) bin split for the log bands, argmax for the peak. The INMP441 is **flat ±3 dB** across our range (datasheet), so there is no per-frequency mic error to compensate — which is *why* a per-band correction table (the kind years of WLED-MM tuning produced) is the wrong tool here. Lesson: for a *new* feature, prefer not to look at prior art at all and let the hardware datasheet + standard DSP be the only reference, so the result is genuinely independent rather than a renamed trace.
