...
- **UDP presence beacon (push, not poll)** — a non-blocking UDP beacon (each device broadcasts "I'm here"; others `recvFrom`-poll, microseconds per tick, no hot-path stall) is the right **presence** layer for projectMM↔projectMM (the MoonLight approach), far cheaper than HTTP polling. HTTP stays the **identify** layer (WLED/HA/generic), mDNS browse the broader-ecosystem layer — see the "Discovery is per-protocol" section in [DevicesModule.md](../moonmodules/core/DevicesModule.md). UDP receive does NOT interfere with LED driving (it's a cheap non-blocking poll, unlike the blocking HTTP probe); UDP *identify* round-trips, if added, belong on the scan task too.
