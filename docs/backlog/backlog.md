...
### ESP32-P4 support — rounds 2-4 (in progress)

Round 1 (board + Ethernet-only, the Waveshare ESP32-P4-NANO with the IP101 PHY) has landed. Remaining rounds, each its own plan + commit:
- **Round 2 — Parlio LED driver.** A `ParlioLedDriver` (sibling of `LcdLedDriver`) using the P4's Parlio TX peripheral for parallel WS2812 — `parlioLanes` SOC-gated flag, `platform_esp32_parlio.cpp`, the same pre-encoded-frame + autonomous-DMA shape as the LCD driver. Must avoid the C6 SDIO pins (GPIO 14-19) and the Ethernet pins (31/50/51/52). No loopback, no strip wired yet.
