...
### DevicesModule — discovery growth (v1 is HTTP scan, boot-only)

DevicesModule v1 does a one-shot HTTP subnet sweep at boot (plus a manual "scan" button); there is deliberately **no periodic background scan** because the probe is a *blocking* `httpGet` running on the render task — each probe stalls the tick up to the probe timeout (~150 ms), which would flicker the LEDs if it ran continuously. Two related improvements unlock safe, richer discovery:

- **Move the probe to its own FreeRTOS task** — run the blocking HTTP probes off the render task entirely, handing results back to the module through a small thread-safe buffer. This is the enabler for both **speed** and **safe periodic** scanning. v1 is slow *by necessity*: the probe blocks ~150 ms on the render task, so it can only do 1 IP/tick (a full /24 takes ~4 min) — bumping IPs/tick or shortening the timeout would either stall the tick (LED flicker) or miss slow responders. On its own task the probe can run many IPs concurrently with a generous timeout and **zero** hot-path impact, making a full sweep seconds instead of minutes AND making continuous background re-scan safe. The boot-only + slow-sweep limitations in v1 both exist solely because the probe is on the render task today; this single change removes both.
- **mDNS browse for discovery (the next discovery strategy)** — the industry-standard way to find devices: browse service types (`_http._tcp` / `_wled._tcp` / `_home-assistant._tcp` / `_apple-midi._udp` / ESPHome / Shelly), one query finds the whole ecosystem, push-style, no subnet sweep. We already advertise mDNS; this adds the *query* side (the ESP-IDF async `mdns_query_async_*` so it never blocks the render task). Becomes the primary discovery path; the HTTP sweep stays as the catch-all for non-advertising hosts (a PC instance on :8080, generic). When this lands, decouple `mdns_init` from the advertise toggle (init stays up for browse even when the device chooses not to advertise — see NetworkModule's `syncMdns`).
- **The four-mechanism split (decided, for when these land)** — discovery and messaging are separate axes, and none replaces another: **mDNS** = discovery (find who's there, standard, whole ecosystem); **HTTP scan** = discovery fallback (what mDNS misses); **REST `/api/control`** = *reliable* messaging (config push, "receive my Art-Net", fleet OTA — TCP guarantees delivery, already built); **UDP** = *lossy real-time streaming* only (SuperSync clock / live timing, where drop-and-continue is fine and low latency matters). The MoonLight "messages sometimes didn't arrive" pain came from using UDP for must-arrive messages — avoid it by routing must-arrive over REST and reserving UDP for streams. A UDP *presence beacon* could also seed projectMM↔projectMM discovery, but mDNS is preferred there for being the recognizable standard. UDP receive is a cheap non-blocking poll (no hot-path stall); UDP *send* of large frames is throughput-bound (see Async ArtNet) and belongs off the render task.
...
- `docs/install/boards.json` is the single source of truth for valid board names. Schema landed as `[{ name, firmwares[] }]` — single `name` field (no key/label split; `name` is both identifier and display label), and `firmwares[0]` is the default the picker pre-selects (no separate `default_firmware` field; reorder the array to change the default).
...
- Picking a board narrows the firmware dropdown to that board's `firmwares[]`, pre-selects the default (precedence: own-firmware-key > localStorage saved > board default = `firmwares[0]` > first compatible), and disables the firmware select when only one option remains.
